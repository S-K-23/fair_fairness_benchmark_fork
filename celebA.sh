...
#
# A self-contained script to run fairness benchmarks on CelebA using ResNet-20.
# Works with the Fair Fairness Benchmark (FFB) directory structure.
#
# Usage:
#   ./run_celeba_resnet20.sh                           # Run ERM baseline
#   ./run_celeba_resnet20.sh --method diffdp --lam 1.0 # Run with specific method
#   ./run_celeba_resnet20.sh --method all              # Run all methods
#
# Requirements:
#   - Place this script in the FFB root directory
#   - CelebA dataset must be in datasets/celeba/raw/
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --lam) LAM="$2"; shift 2 ;;
        --target_attr) TARGET_ATTR="$2"; shift 2 ;;
        --sensitive_attr) SENSITIVE_ATTR="$2"; shift 2 ;;
        ...
        -h|--help)
            ...
            echo ""
            echo "Options:"
            echo "  --method        Fairness method: erm, diffdp, diffeopp, diffeodd, hsic, pr, all (default: erm)"
            echo "  --lam           Lambda for fairness trade-off (default: 1.0)"
            echo "  --target_attr   Target attribute: Smiling, Wavy_Hair, Attractive (default: Smiling)"
            echo "  --sensitive_attr Sensitive attribute: Male, Young (default: Male)"
            echo "  --batch_size    Batch size (default: 128)"
            echo "  --lr            Learning rate (default: 0.001)"
            echo "  --num_steps     Number of training steps (default: 500)"
            echo "  --seed          Random seed (default: 1314)"
            echo "  --log_freq      Logging frequency (default: 10)"
            exit 0
            ;;
...
echo "=========================================="
echo "CelebA + ResNet-20 Fairness Benchmark"
echo "=========================================="
echo "Method: $METHOD"
echo "Lambda: $LAM"
echo "Target: $TARGET_ATTR"
echo "Sensitive: $SENSITIVE_ATTR"
echo "=========================================="

# Check for pytorchcv and install if needed
echo "Checking for pytorchcv..."
python -c "import pytorchcv" 2>/dev/null || {
    echo "Installing pytorchcv..."
    pip install pytorchcv
}

# Create the Python benchmark script
cat > /$SCRIPT_DIR/celeba_resnet20_benchmark.py << 'PYTHON_SCRIPT'
#!/usr/bin/env python
"""
CelebA + ResNet-20 Fairness Benchmark
...
run_method() {
    ...
    echo ""
    echo "=========================================="
    echo "Running: $method (lambda=$LAM)"
    echo "=========================================="

    python /tmp/celeba_resnet20_benchmark.py \
        --method "$method" \
        --lam "$LAM" \
        ...
        --seed "$SEED" \
        --log_freq "$LOG_FREQ" \
        --data_path "datasets/celeba/raw/"
}

# Run based on method selection
if [ "$METHOD" = "all" ]; then
    echo "Running all fairness methods..."
    for m in erm diffdp diffeopp diffeodd hsic pr; do
        run_method "$m"
    done
else
    run_method "$METHOD"
